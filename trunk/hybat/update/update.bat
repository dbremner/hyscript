...
rem 使用介绍
rem 功能:试图设置自动连接清华的Windows Update服务器
...
if %Language%==Chinese (
 echo [INFO] 本程序专为设置清华的的Windows Automatic Updates 服务
) else (
 echo [INFO] This will setup TSINGHUA Windows Automatic Updates Services
)
...
reg add HKLM\Software\Policies\Microsoft\Windows\WindowsUpdate /v WUServer /t REG_SZ /d "http://wsus.tsinghua.edu.cn" /f >NUL
reg add HKLM\Software\Policies\Microsoft\Windows\WindowsUpdate /v WUStatusServer /t REG_SZ /d "http://wsus.tsinghua.edu.cn" /f >NUL
...
set downloadurl=http://wsus.tsinghua.edu.cn/script/%patchfile%
call :Download
